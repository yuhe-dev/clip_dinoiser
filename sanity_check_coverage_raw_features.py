...
COVERAGE_FEATURE_KEYS = ("knn_neighbor_distances_raw", "prototype_distances_raw")


def load_coverage_records(records_path: str) -> List[Dict[str, object]]:
...
def compute_coverage_bundle_summary(records: Sequence[Dict[str, object]], sample_limit: int = 3) -> Dict[str, object]:
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Summarize extracted raw coverage features.")
    parser.add_argument("--records-path", default="./data/data_feature/coverage/coverage_raw_features.npy")
    parser.add_argument("--out-json", default="./data/data_feature/coverage/coverage_raw_summary.json")
...
def main() -> None:
    ...
    print(f"[coverage-sanity] records_path={records_path}")
    if not os.path.exists(records_path):
        raise FileNotFoundError(f"Raw feature file not found: {records_path}")

    records = load_coverage_records(records_path)
    summary = compute_coverage_bundle_summary(records, sample_limit=args.sample_limit)
    os.makedirs(os.path.dirname(out_json), exist_ok=True)
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(summary, f, indent=2, ensure_ascii=False)

    print(f"[coverage-sanity] saved summary: {out_json}")
    for feature_key, feature_summary in summary["features"].items():
        print(f"[coverage-sanity] {feature_key}: {feature_summary}")
